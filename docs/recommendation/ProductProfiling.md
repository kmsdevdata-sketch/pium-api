...
추천 엔진은 이 원본 데이터를 직접 소비하지 않고, Product Profiling 계층이 생성한 trait/evidence/confidence를 소비한다.
...
이미지는 사용자 노출을 위한 데이터이며,  
초기 추천 판단의 근거로 사용하지 않는다.
...
| ACTIVE_INTENSITY_RISK | 강한 기능성 성분 강도 |
