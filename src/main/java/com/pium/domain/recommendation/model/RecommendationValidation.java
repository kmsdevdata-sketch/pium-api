...
final class RecommendationValidation {

    private RecommendationValidation() {
    }

    static String normalizeRequired(String value, RecommendationErrorCode errorCode) {
    ...
    static <T> List<T> copyRequired(List<T> values, RecommendationErrorCode errorCode) {
        if (values == null) {
            throw new RecommendationException(errorCode);
        }
